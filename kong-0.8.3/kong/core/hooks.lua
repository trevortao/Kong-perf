...
--trevor: mru cache for quick lookup
...
local function invalidate(message_t)
  ...
  elseif message_t.collection == "apis" then
    if message_t.entity then
      ...
      --trevor: delete the corresponding mru cache entry
      mru_cache.delete(mru_cache.api_key(message_t.entity.id))
    end
    cache.delete(cache.all_apis_by_dict_key())
    mru_cache.delete(mru_cache.wildcard_apis_by_dict_key())
